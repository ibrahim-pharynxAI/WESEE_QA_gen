    ...
    def load_checkpoint(self):
        ...
        if os.path.exists(self.checkpoint_file):
            try:
                ...
                print("Warning: Could not load checkpoint file, starting fresh")
    ...
    def process_single_pdf(self, pdf_path):
        ...
        try:
            ...
            md_path = os.path.join(
                "QA_generator/parser/parsed_outputs", f"{pdf_name}_parsed.md"
            )
            questions_output = os.path.join(
                "question_outputs", f"{pdf_name}_questions.json"
            )
            ...
            if os.path.exists(qa_output_file):
                ...
                try:
                    with open(questions_output, "r") as f:
                        ...
                        total_questions = sum(
                            len(item["questions"]) for item in questions_data
                        )
                    ...
                    if answered_questions >= total_questions:
                        ...
                        print(
                            f"✓ Answers already complete: {qa_output_file} ({answered_questions}/{total_questions} questions)"
                        )
                    else:
                        print(
                            f"⚠️ Partial answers found: {qa_output_file} ({answered_questions}/{total_questions} questions). Continuing generation..."
                        )
                except Exception as e:
                    print(
                        f"⚠️ Could not verify answer completion status: {e}. Regenerating answers..."
                    )
            else:
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", default="pipeline_checkpoint.json", help="Checkpoint file path"
    )
    ...
    if os.path.isfile(input_path) and input_path.lower().endswith(".pdf"):
        ...
        if pipeline.is_file_processed(input_path) and args.resume:
            print(
                f"✓ {os.path.basename(input_path)} already processed. Use --resume to reprocess."
            )
            return
